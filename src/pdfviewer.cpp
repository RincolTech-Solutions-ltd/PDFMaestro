...
PdfViewer::PagePdfPos PdfViewer::viewportToPdf(QPointF vpPt) const {
    if (m_pageItems.isEmpty()) return {0, 0.0, 0.0};

    // mapToScene() derives the scene position from the live viewport transform
    // (current scroll + zoom) — always exact, never stale.
    QPointF sc = mapToScene(vpPt.toPoint());

    // Find which page contains the point, or the closest by vertical distance.
    int    pageIdx = 0;
    double best    = 1e18;
    for (int i = 0; i < m_pageItems.size(); ++i) {
...
void PdfViewer::addSigOverlayAtViewport(const QImage& img, QPointF vpCenter,
                                         double sigWPt, double sigHPt)
{
    auto [pageIdx, xPdf, yPdf] = viewportToPdf(vpCenter);
    // Centre the sig on the click point.
    addSigOverlay(img, pageIdx, xPdf - sigWPt / 2.0, yPdf - sigHPt / 2.0, sigWPt, sigHPt);
}
